    ...
    try {
        ...
        if (data.development && data.development.verificationCode) {
            ...
            if (devCodeDisplay) {
                ...
                const oldVisibleCode = document.getElementById('visibleCodeDisplay');
                if (oldVisibleCode) {
                    oldVisibleCode.remove();
                }
                
                // Находим второй параграф с текстом "Проверьте почту..."
                const infoMessage = devCodeDisplay.closest('.info-message') || devCodeDisplay.parentElement;
                const verificationCodeSection = document.getElementById('verificationCodeSection');
                
                // Вариант 1: Вставляем код прямо в текст второго параграфа
                if (infoMessage) {
                    const paragraphs = infoMessage.querySelectorAll('p');
                    if (paragraphs.length >= 2) {
                        const secondParagraph = paragraphs[1];
                        // Добавляем код прямо после текста параграфа
                        const codeSpan = document.createElement('span');
                        codeSpan.style.cssText = 'display: inline-block !important; margin-left: 10px; padding: 5px 10px; background: #fff3cd !important; border: 2px solid #856404 !important; border-radius: 5px !important; font-size: 20px !important; font-weight: bold !important; color: #856404 !important; font-family: monospace !important; letter-spacing: 2px !important;';
                        codeSpan.textContent = data.development.verificationCode;
                        secondParagraph.appendChild(codeSpan);
                        console.log('✅ Код вставлен в текст второго параграфа');
                    }
                }
                
                // Вариант 2: Создаем большой заметный блок ПЕРЕД формой
                const verifyEmailForm = document.getElementById('verifyEmailForm');
                if (verifyEmailForm && verificationCodeSection) {
                    ...
                    codeAlert.style.cssText = `
                        ...
                        border: 4px solid #856404 !important;
                    ...
                    `;
                    codeAlert.innerHTML = `
                        <div style="font-size: 16px; margin-bottom: 15px; color: #856404; font-weight: bold;">
                            🔐 КОД ПОДТВЕРЖДЕНИЯ
                        </div>
                        <div style="font-size: 48px; font-weight: bold; color: #856404; letter-spacing: 8px; font-family: 'Courier New', monospace; margin: 20px 0; text-shadow: 2px 2px 4px rgba(0,0,0,0.2);">
                            ${data.development.verificationCode}
                        </div>
                        <div style="font-size: 14px; margin-top: 15px; color: #856404; opacity: 0.9;">
                            Введите этот код в поле выше для завершения регистрации
                        </div>
                    `;
                    verificationCodeSection.insertBefore(codeAlert, verifyEmailForm);
                    console.log('✅ Большой блок с кодом создан перед формой');
                }
