  ...
  try {
    ...
    const sendEmailWithTimeout = Promise.race([
      ...
      new Promise((_, reject) => {
        setTimeout(() => {
          reject(new Error('Таймаут отправки email (15 секунд)'));
    ...
    // Пытаемся дождаться отправки (но не блокируем ответ дольше 5 секунд)
    try {
      await Promise.race([
        sendEmailWithTimeout,
        new Promise((resolve) => setTimeout(resolve, 5000)) // Максимум 5 секунд ждем
      ]);
    } catch (e) {
      // Игнорируем ошибку - код все равно вернем
      console.warn('Не удалось дождаться результата отправки email, продолжаем...');
    }
